...
public class KillAura extends Module {
    ...
    public final BooleanSetting onlySpace = new BooleanSetting("Умные криты", true);
    public final BooleanSetting noAttackWhileEating = new BooleanSetting("Не бить когда ешь", false);
    public final ModeSetting wallMode = new ModeSetting("Бить через стены", "Выкл", "Выкл", "Полные", "Неполные");
    ...
    private void onGameUpdate(EventGameUpdate e) {
        ...
        if (isResolving) {
            if (resolverTimer.isReached(300)) {
                ...
                RotationComponent.update(rot, 360, 360, 360, 360, 0, 1, false);
                lastYaw = rot.getYaw();
    ...
    private void onUpdate(final EventTick ignored) {
        ...
        if (target != null) {
            ...
            Vec3d predict = PredictUtils.getPredicted(target, 3);
    ...
    private boolean isValidEntity(Entity entity) {
        ...
        if (wallMode.is("Неполные")) {
            Vec3d eyePos = player.getEyePos();
            Vec3d targetPoint = BestPoint.getNearestPoint(entity);
            if (RaytraceUtil.hasFullBlockInPath(eyePos, targetPoint)) {
                return false;
            }
        }
    ...
    public boolean canAttack() {
        ...
        if (noAttackWhileEating.getValue() && mc.player.isUsingItem()) return false;
        ...
        if (target.isGliding()) {
            double distToPredict = player.getEyePos().distanceTo(PredictUtils.getPredicted(target, 3));
    ...
    private void updateVanillaRotation(LivingEntity target) {
        ...
        RotationComponent.update(rotation, 360, 360, 360, 360, 0, 1, false);
    }
    ...
    private void updateSlothRotation(LivingEntity target) {
        ...
        if (target.isGliding() && !isTurnaroundActive) {
            targetPoint = PredictUtils.getPredicted(target, 3);
        }
        ...
        RotationComponent.update(smoothRot, 360, 360, Math.abs(deltaYaw2) > 3 || Math.abs(deltaPitch2) > 3 ? 0 : 360, Math.abs(deltaYaw2) > 3 || Math.abs(deltaPitch2) > 3 ? 0 : 360, 0, 1, false);
    ...
    private void updateLonyJirRotation(LivingEntity target) {
        ...
        var predict = PredictUtils.getPredicted(target, 3 + 2.5f);

        if (target.isGliding() && !isTurnaroundActive) angle = new Rotation(predict);
        ...
        if (mc.player.isGliding() && target.isGliding())
            RotationComponent.update(smoothRot, 360, 360, 360, 360, 0, 1, false);
        lastYaw = smoothRot.getYaw();
        lastPitch = smoothRot.getPitch();
    }
    ...
    private void slothTest(LivingEntity target) {
        ...
        if (target.isGliding() && !isTurnaroundActive) {
            point = PredictUtils.getPredicted(target, 3);
        }
        ...
        RotationComponent.update(legitRot, 360, 360, 360, 360, 0, 1, false);
    ...
    private void updateLonyGriefRotation(LivingEntity target) {
        Vec3d point = target.isGliding() && !isTurnaroundActive ? PredictUtils.getPredicted(target, 3) : resolveMultipoint(target, BestPoint.getPoint(target), 6);
    ...
    private void updateWellmineRotation(LivingEntity target) {
        ...
        if (target.isGliding() && !isTurnaroundActive) {
            vector = PredictUtils.getPredicted(target, 3);
        }
        ...
        if (!back) {
            ...
            if (mc.player.isGliding() && target instanceof PlayerEntity && target.isGliding()) {
                offset = PredictUtils.getPredicted(target, 3);
            }
    ...
    private Vec3d resolveMultipoint(LivingEntity target, Vec3d point, double range) {
        ...
        if (wallMode.is("Полные")) {
            return point;
        }

        if (wallMode.is("Неполные")) {
            Vec3d eyePos = mc.player.getEyePos();
            if (!RaytraceUtil.hasFullBlockInPath(eyePos, point)) {
                return point;
            }
        }
    ...
    private void renderPredictPoint(MatrixStack matrices, Camera camera, float tickDelta) {
        ...
        Vec3d predictPos = PredictUtils.getPredictedRender(target, 3, tickDelta);
